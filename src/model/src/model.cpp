...
Model::Model(const std::string &filename) : vertices(), faces() {
	...
	std::vector<float3> vertices_host{uvertices};
	std::vector<float3> normals_host{uvertices};
	std::vector<float2> textures_host{uvertices};
	...
	for (unsigned char num_face_vertice : shapes[0].mesh.num_face_vertices) {
		...
		for (size_t v = 0; v < fv; v++) {
			...
			vertices_host[idx.vertex_index] = float3{vx, vy, vz};
			...
			if (idx.normal_index >= 0) {
				...
				normals_host[idx.vertex_index] = float3{nx, ny, nz};
			}
			...
			if (idx.texcoord_index >= 0) {
				...
				textures_host[idx.vertex_index] = float2{tx, ty};
				has_textures = true;
			}
		}
	...
	checkCudaErrors(cudaMalloc((void**)(&normals), sizeof(float3) * n_vertices));

	if (has_textures)
		checkCudaErrors(cudaMalloc((void**)(&textures), sizeof(float3) * n_vertices));
	...
	checkCudaErrors(cudaMemcpy(normals, normals_host.data(), n_vertices * sizeof(float3), cudaMemcpyHostToDevice));

	if (has_textures)
		checkCudaErrors(cudaMemcpy(textures, textures_host.data(), n_vertices * sizeof(float2), cudaMemcpyHostToDevice));
